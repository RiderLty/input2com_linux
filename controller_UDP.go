...
func (mk *UDPMouseKeyboard) MouseMove(dx, dy, Wheel int32) error {
	...
	if err != nil {
		logger.Errorf("UDPMouseKeyboard MouseMove error : %v", err)
		return err
	}
	return nil
}

func (mk *UDPMouseKeyboard) MouseBtnDown(keyCode byte) error {
	var err error = nil
	mk.mu.Lock()
	defer mk.mu.Unlock()
	switch keyCode {
...
func (mk *UDPMouseKeyboard) MouseBtnUp(keyCode byte) error {
	var err error = nil
	mk.mu.Lock()
	defer mk.mu.Unlock()
	switch keyCode {
...
func (mk *UDPMouseKeyboard) KeyDown(keyCode byte) error {
	mk.mu.Lock()
	defer mk.mu.Unlock()
...
func (mk *UDPMouseKeyboard) KeyUp(keyCode byte) error {
	mk.mu.Lock()
	defer mk.mu.Unlock()
